...
task "debug-build", "build compiler":
  if not existsDir("bin"):
    createDir("bin")
  discard execShellCmd("nim c --cpu:i386 --passC:-m32 --passL:-m32 compiler/flori")
  copyFile("compiler/flori".exe, "bin/flori".exe)
task "build", "build compiler":
  if not existsDir("bin"):
    createDir("bin")
  discard execShellCmd("nim c -d:release --cpu:i386 --passC:-m32 --passL:-m32 compiler/flori")
  copyFile("compiler/flori".exe, "bin/flori".exe)
...
task "test", "":
  discard execShellCmd("nim c -r --cpu:i386 --passC:-m32 --passL:-m32 tests/basictester.nim")
